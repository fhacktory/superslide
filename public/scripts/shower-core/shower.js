...
window.shower = (function(window, document, undefined) {
	...
	*/
	...
	shower.init = function(slideSelector, progressSelector) {
		...
		if (shower.debugMode) {
			presentation.classList.add('debug');
		...
		presentation.appendChild(liveRegion);
	...
	shower.run = function() {
		var currentSlideNumber = shower.getCurrentSlideNumber(),
			isSlideMode = presentation.classList.contains('full') || shower.isSlideMode();
	...
	*/
	shower._getTransform = function() {
		var offset = 0;
		if(shower.isControllerMode())
			offset = 80;
		var denominator = Math.max(
			presentation.clientWidth / (window.innerWidth-offset),
			presentation.clientHeight / window.innerHeight
		);
	...
	shower._applyTransform = function(transform) {
		[
			...
			'transform'
				...
				presentation.style[prop] = transform;
		});
	...
	*/
	shower._getSlideIdByEl = function(el) {
		while ('BODY' !== el.nodeName && 'HTML' !== el.nodeName && 'presentation' !== el.id) {
	...
	*/
	shower.enterSlideMode = function(callback) {
		...
		presentation.classList.remove('list');
		presentation.classList.add('full');
	...
	*/
	shower.enterListMode = function(callback) {
		if(shower.isClientMode())
			return false;
		var currentSlideNumber;

		// Anyway: change body class (@TODO: refactoring)
		presentation.classList.remove('full');
		presentation.classList.add('list');
	...
	*/
	shower.isListMode = function() {
		if (isHistoryApiSupported) {
			...
			return presentation.classList.contains('list');
		}
	};
	...
	*/
	shower.isSlideMode = function() {
		if (isHistoryApiSupported) {
			...
			return presentation.classList.contains('full');
		}
	};

	/**
	* Check if it's Controller mode.
	* @returns {Boolean}
	*/
	shower.isControllerMode = function() {
		return document.body.classList.contains('controllerMode');
	}
	...
	* @returns {Boolean}
	*/
	shower.isClientMode = function() {
		return document.body.classList.contains('clientMode');
	}
